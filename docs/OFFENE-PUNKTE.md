...
- [ ] **Walkie-Geräteton läuft NICHT durch den Raum-Portal-Graphen:** `WalkieDeviceOutput` dämpft nach reiner Luftlinie (MaxHearingDistance, `(1−d/max)²`). Der Leak/Sidetone am Gerät ist aber physischer Raum-Schall — durch geschlossene Türen/Wände sollte er dumpf/leiser sein, so wie die Mund-Stimme (`VoicePipeline` + `GraphModifier`). Entscheidung offen: `WalkieDeviceOutput` an `VoiceGraph.TryFindPath` anschließen (eigene Zone-Ermittlung + Pfad-Länge statt Luftlinie). Der Graph-Debug-HUD (F7) macht genau das sichtbar.
