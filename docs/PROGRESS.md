...
Status: **v18.2 (resolver-fix) — WALKIE-REMOTE-BETRIEB VERIFIZIERT (2026-09-20, 13:10-Lauf, Editor-Host + Build)**: Der erste Lauf-5-Versuch (12:05–12:31) scheiterte an DREI unabhängigen Root Causes: (1) NGO-String-NetworkVariable-Serialisierung ohne `WriteValueSafe`/`ReadValueSafe` warf `OverflowException` → Remote-PTT-Sync kam nie an (Spiel, `WalkieWorldItem`); (2) der `VoiceChannelResolver`-Lookup auf `Sessions` fand die explizite Interface-Implementierung nie → immer Lobby-Fallback (Package v18.2, Commit b50626a); (3) verwaiste UGS-Lobby-Mitgliedschaften blockierten Joins („already a member", „Session voll" — Spiel, `RelaySessionUI`: Cleanup vor Join + Quit-Cleanup). Verifikation: Join fehlerfrei, `Netz-Sync angekommen … lokal=False` in beide Richtungen, `WALKIE REMOTE FEED Signal peak=0,299`, `OUTPUT AN mode=REMOTE reason=AUDIBLE`. Beweiskette: `walkie-talkie-debug-history.md` Abschnitt 22; Nacharbeiten (echter Remote-Test mit Freund, Resolver-Timing, Cleanup-Gegenprobe, Spiel-Commits): `OFFENE-PUNKTE.md`.
...
- [x] PTT-Wiederholbarkeit: Registry verfolgt die exakte Senderinstanz; ein einzelner Capture-Tap bleibt DSP-aktiv, Filter-Nullung plus Source-Mute sperren seinen direkten 2D-Ausgang, und kanonische Kanal-IDs bleiben allokationsfrei
- [x] Capture-Diagnose: Logs unterscheiden Eingangssignal, DSP-Callbacks, finalen direkten Source-Ausgang, Mute-Invariante, Revision und aktive/verfügbare Vivox-Audiogeräte
- [ ] Hörtest: zwei Walkies in der Testszene / im Spiel, alle Verhaltensregeln einzeln verifiziert — Teil 1 am 13.10-Lauf 2026-09-20 verifiziert (Remote-PTT in beide Richtungen inkl. Audio am Gerät, `OUTPUT AN reason=AUDIBLE`); Rest (Distanz-Falloff, Mund-Stimme bei PTT, Half-Duplex/First-Speaker) beim Remote-Test
