...
namespace Earshot.Voice
{
    ...
    /// Netzwerk-Tap). Dafuer braucht es weiterhin zwei echte Teilnehmer - siehe
    /// <c>-profile NAME</c> in <see cref="CoopServices"/> fuer den Solo-Test mit zwei
    /// Fenstern auf einem PC.
    /// </para>
    /// </summary>
    [AddComponentMenu("Earshot/Voice Test Speaker")]
    public class VoiceTestSpeaker : MonoBehaviour
    {
        [SerializeField]
        [Tooltip("Aus, sobald eine echte Internet-Sitzung laeuft. Verhindert, dass ein echter Mitspieler den Testton hoert.")]
        private bool onlyWithoutRealSession = true;
        ...
        private void Update()
        {
            // Der Zustandswechsel schaltet ueber das Event sofort ab. Dieses Polling ist
            // nur ein zweites Sicherheitsnetz - falls ein Aufruf je verpasst wird, dauert
            // es hoechstens SafetyRecheckSeconds, bis ein echter Mitspieler garantiert
            // nichts vom Testton mitbekommt.
        ...
        private void Refresh()
        {
            ...
            bool shouldRun = !onlyWithoutRealSession || !Coop.IsInSession || Coop.IsLocalSession;
            if (shouldRun) Ensure();
            else Remove();
        }
        ...
        private void Ensure()
        {
            ...
            // egal wie oft Ensure/Remove beim Wechseln des Sitzungszustands laufen.
            host = new GameObject("Voice Test Speaker (laufend)");
            host.transform.SetParent(transform, false);

            var source = host.AddComponent<AudioSource>();
            emitter = host.AddComponent<VoiceEmitter>();
            emitter.Initialize("debug:" + speakerId, source, CoopSettings.Instance.VoiceProfile);
            ...
            CoopLog.Info($"Voice-Testton '{speakerId}' laeuft an {name}.");
        }
