...
{
    /// <summary>
    /// Lauscht der laufenden Vivox-Verbindung und schreibt Rede-/Hoer-Ereignisse ins
    /// Voice-Log (<see cref="VoiceSessionLog"/>, Start/Ende der Sitzung uebernimmt bereits
    /// <see cref="EarshotVoice"/> selbst). Aendert den Klang nicht, zaehlt nur mit.
    /// </summary>
    [AddComponentMenu("Earshot Voice/Voice Session Recorder")]
    ...
    public sealed class VoiceSessionRecorder : MonoBehaviour
    {
        ...
        {
            VoiceRoster.PlayerAdded += OnPlayerAdded;
            VoiceRoster.PlayerRemoved += OnPlayerRemoved;
        }
        ...
        private static void OnPlayerAdded(IProximityVoicePlayer player)
        {
            if (player == null) return;
            VoiceSessionLog.Note(
                $"SPIELER da: {player.DisplayName}  lokal={player.IsLocalPlayer}  id={player.PlayerId}");
        }

        private static void OnPlayerRemoved(IProximityVoicePlayer player)
        {
            if (player == null) return;
        ...
        private void Update()
        {
            bool connected = EarshotVoice.IsConnected;
            ...
            if (!connected || !VoiceSessionLog.IsRecording) return;
        ...
        private void PollMachine()
        {
            ...
            float heard = EarshotVoice.HeardVoiceVolume;
        ...
        {
            var profile = EarshotVoiceSettings.Instance != null ? EarshotVoiceSettings.Instance.VoiceProfile : null;
            string range = profile != null ? profile.MaxHearingDistance.ToString("0") + " m" : "?";
            var local = VoiceRoster.LocalPlayer;
            string pos = local != null
                ? $"x{local.Position.x:0.0} y{local.Position.y:0.0} z{local.Position.z:0.0}"
                : "kein Avatar";
            ...
            VoiceSessionLog.Note(
                ...
                "  Gehoert-Regler " + EarshotVoice.HeardVoiceVolume.ToString("0.00") +
                "  Mic " + (EarshotVoice.MicrophoneMuted ? "STUMM" : "an") +
                "  Fokus " + (Application.isFocused ? "ja" : "nein") +
                "  Profil " + range +
                "  In='" + EarshotVoice.ActiveInputDeviceName + "'" +
                "  Out='" + EarshotVoice.ActiveOutputDeviceName + "'" +
                "  Pos " + pos);
        }
        ...
        {
            var local = VoiceRoster.LocalPlayer;
            if (local == null) return "(kein lokaler Avatar)";
            return local.VoiceAnchor != null ? "Kopf" : "Avatar";
        }
        ...
        private static string DisplayName(bool self, string playerId)
        {
            ...
            if (VoiceRoster.TryGetByPlayerId(playerId, out var player) && player != null)
            {
