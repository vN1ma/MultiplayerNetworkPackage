...
{
    /// <summary>
    /// Oeffentliche Schnittstelle des Proximity-Chats. Im Gegensatz zum vollen Earshot-Paket
    /// haengt sich diese Schicht NICHT selbst an eine Sitzung. Das Host-Spiel ruft
    /// <see cref="ConnectAsync"/> auf, sobald alle im selben Match sind, und
    /// <see cref="DisconnectAsync"/> beim Verlassen.
    /// </summary>
    public static class ProxVoice
    {
        ...
        /// Genau diese Zeichenkette muss auf dem eigenen <see cref="ProxVoicePlayer"/>
        /// liegen und zu den Mitspielern synchronisiert werden.
        /// </summary>
        ...
        public static async Task SetInputDeviceAsync(string deviceName)
        {
            ...
            catch (Exception ex)
            {
                ...
                ProxLog.Exception("Mikrofon wechseln fehlgeschlagen", ex);
            }
        }
        ...
        public static async Task SetOutputDeviceAsync(string deviceName)
        {
            ...
            catch (Exception ex)
            {
                ...
                ProxLog.Exception("Lautsprecher wechseln fehlgeschlagen", ex);
            }
        }
        ...
        /// Null = Kopf des lokalen <see cref="ProxVoicePlayer"/>, sonst aktiver AudioListener.
        /// </summary>
        ...
        public static async Task EnsureSignedInAsync(bool signInAnonymouslyIfNeeded = true)
        {
            ...
            if (!signInAnonymouslyIfNeeded)
            {
                throw new InvalidOperationException(
                    "Unity Authentication ist nicht angemeldet. Vor ProxVoice.ConnectAsync " +
                    "selbst anmelden, oder signInAnonymouslyIfNeeded auf true lassen.");
            }

            // Das Hotel-Spiel loggt oft parallel ein. Zwei SignIn gleichzeitig werfen
            // "already signing in" — dann warten wir, statt ein zweites Mal anzustossen.
            try
            {
                await auth.SignInAnonymouslyAsync();
                ProxLog.Info("Anonym bei Unity Authentication angemeldet. PlayerId=" + LocalPlayerId);
            }
            catch (Exception)
            {
                int waitedMs = 0;
        ...
        {
            var settings = ProxVoiceSettings.Instance;
            if (!settings.VoiceEnabled)
            {
                ProxLog.Info("Voice ist in den Einstellungen deaktiviert.");
                return;
            }

            if (string.IsNullOrWhiteSpace(channelName))
            {
                ProxLog.Warn("Kein Kanalname. Alle Spieler brauchen denselben Namen fuer denselben Match.");
                return;
            }
            ...
            ProxLog.Info(
                "Proximity-Chat verbunden. Kanal='" + channelName +
