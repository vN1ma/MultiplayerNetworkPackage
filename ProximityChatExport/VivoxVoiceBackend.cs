    ...
    public class VivoxVoiceBackend : IVoiceBackend, IVoiceBackendRecovery
    {
        ...
        public bool MicrophoneMuted
        {
            ...
            set
            {
                ...
                ProxLog.Info(value ? "Mikrofon stummgeschaltet." : "Mikrofon aktiv.");
            }
        }
        ...
        public async Task ConnectAsync(string channel, string displayName)
        {
            ...
            ProxLog.Info($"Sprachkanal '{channel}' wird betreten.");
        ...
        public async Task DisconnectAsync()
        {
            ...
            if (!string.IsNullOrEmpty(channelName))
            {
                ...
                try
                {
                    ...
                    ProxLog.Info("Sprachkanal verlassen.");
                }
                catch (Exception ex)
                {
                    ProxLog.Warn($"Sprachkanal konnte nicht sauber verlassen werden: {ex.Message}");
                }
            }
        }
        ...
        private async Task EnsureLoggedInAsync(string displayName)
        {
            ...
            ProxLog.Info("Anmeldung bei Vivox laeuft.");
        ...
        private void TryCreateTap(VivoxParticipant participant)
        {
            ...
            try
            {
                ...
                ProxLog.Info($"Stimme empfangen von {participant.PlayerId}.");
                SpeakerAdded?.Invoke(new VoiceSpeaker(participant.PlayerId, source));
            }
            catch (Exception ex)
            {
                ProxLog.Exception($"Audio Tap fuer {participant.PlayerId} fehlgeschlagen", ex);
            }
        }
        ...
        private void OnParticipantRemoved(VivoxParticipant participant)
        {
            ...
            ProxLog.Info($"Stimme verstummt: {participant.PlayerId}.");
        ...
        void IVoiceBackendRecovery.RecoverSpeaker(string playerId)
        {
            ...
            {
                ProxLog.Exception($"Alten Tap fuer {playerId} loeschen fehlgeschlagen", ex);
            }
